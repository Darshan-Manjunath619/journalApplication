...
export function EditJournalPage() {
  ...
  if (journalId === null || (journal.error instanceof ApiError && journal.error.status === 404)) {
    return <FeedbackPanel title={'Journal not found'}>The journal does not exist or is not available to your account.</FeedbackPanel>
  }
  if (journal.isPending) return <FeedbackPanel role={'status'} title={'Loading journal'}>Please wait...</FeedbackPanel>
  if (journal.isError) return <FeedbackPanel role={'alert'} title={'Unable to load journal'} actionLabel={'Try again'} actionDisabled={journal.isFetching} onAction={() => journal.refetch()}>Your journal could not be loaded.</FeedbackPanel>
